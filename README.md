...
It is both a structured course and a reference implementation. The notebooks move from basic retrieval pipelines to advanced retrievers, research-inspired RAG patterns, LangGraph agentic workflows, RAGAS evaluation, and a production-style capstone.
...
- A capstone system with ingestion, hybrid retrieval, reranking, orchestration, and evaluation
...
| 33 | `Production_RAG_Capstone.ipynb` | Full capstone system | M10 |
...
The Module 10 capstone ships with standalone Python modules:

```bash
# Ingest documents into a local Chroma vector store
uv run python Module_10_Capstone/ingestion.py --docs_dir ./data --collection rag_capstone
...
```python
from Module_10_Capstone.pipeline import build_rag_pipeline
